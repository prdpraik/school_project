     ...
     if(!in_array(1,array($this->session->userdata('admin_login'),$this->session->userdata('accountant_login'))))
        ...
        if ($param1 == 'create') {
           ...
           // $data['password'] = $this->input->post('password');
			...
			$data['student_category'] = $this->input->post('student_category');
...
for ($row =2,$i=0; $row <= $highestRow; $row++){ 
 ...
 $gender=$rowData[$i][5];
 $nationality=$rowData[$i][6];
 $present_address=$rowData[$i][7];
 $permanent_address=$rowData[$i][8];
 $father_name=$rowData[$i][9];
 $mother_name=$rowData[$i][10];
 
 //$blood_group=$rowData[$i][6];

 $parent_phone1=$rowData[$i][11];
 $parent_phone2=$rowData[$i][12];
 //$parent_email=$rowData[$i][14];
 $occupation=$rowData[$i][13];
 $income=$rowData[$i][14];
 $previous_school=$rowData[$i][15];
 $rte=$rowData[$i][16];
 if($rte=='Yes'){
	 $rte=1;
 }else {
...
if( (!empty($mother_name)) && ($mother_name!="") && (!empty($birthplace)) && (!empty($birthplace))){        
if((!empty($nationality)) && (!empty($nationality))){  
     
if(1){ 
         
if((!empty($parent_phone1)) && (!empty($parent_phone1))){         
...
if( (!empty($permanent_address)) && ($permanent_address!="")  ){  
    $query = $this->db->query("select roll from student where roll='$roll_no' and class_id='$class'");
      ...
      if($rowcount=='0'){
         ...
         $query = $this->db->query("insert into student(admission_number,roll,name,sex,birthday,father_name,
     mother_name,birth_place,nationality,
     parent_phone1,parent_phone2,occupation,income_per_annum,
     address,permanent_address,previous_school_name,rte,aadhaar_number,class_id) values('$admission_number','$roll_no','$name',
        '$gender','$dob','$father_name','$mother_name','$birthplace',
         '$nationality'
         ,'$parent_phone1','$parent_phone2','$occupation',
         '$income','$present_address','$permanent_address','$previous_school','$rte','$adhar','$class')");

         echo $details['item'][]="<div style='color:#1171A3;'>The Roll No $roll_no inserted successfully... </div>"; 
      }
      else{
          echo $details['item'][]="<div style='color:#f00;'>The Roll No $roll_no already Exists.</div> ";
      }
...
echo '<a href="'.base_url().'index.php?admin/student/'.$class.'">click here to reload data</a>';
exit;
//echo $counter;
}

	
//$_SESSION['data']=$details;
...
/*     * **MANAGE STAFF**** */
    ...
    function system_settings($param1 = '', $param2 = '', $param3 = '') {
		...
		$page_data['avaibale_balance'] = getSMSCount();
...
// E-Books Customization
 ...
 function ebookscat($param1 = '', $param2 = ''){
	...
	***************************************************************************/
	...
	function add_incomeexpenses($id=0){
		$this->load->model('fee_model');
		$date=explode("/",$this->input->post('entry_date'));
		$data['entry_date'] = $date[2].'-'.$date[1].'-'.$date[0];
		$data['income_exp_name']=$this->input->post('income_exp_name');
		$data['amount']=$this->input->post('amount');
		$data['category_description']=$this->input->post('category_description');
		$data['entry_type']=$this->input->post('entry_type');
		$this->fee_model->put_income_exp($data,$id); 
		redirect(base_url().'index.php?admin/income_expenses','refresh');
	}
...
<div class="controls">';
	...
	function fee_particular_insert(){
		...
		$fpdata['fee_type'] = $this->input->post('fee_type');
		//$fpdata['fee_applicble'] = $this->input->post('fee_applicble');
		$fpdata['fee_applicble'] = implode(",",$_POST['fee_applicble']);
		//print_r($fpdata);
	...
	function fee_particular_update(){
		...
		$fpdata['fee_applicble'] = implode(",",$_POST['fee_applicble']); //$this->input->post('fee_applicble');
		//echo '<pre>';
		//print_r($fpdata); exit;
    ...
    function feecollectall(){
        ...
        $page_data['page_name']  = 'feecollectall';
        $page_data['page_title'] = get_phrase('fee_collect');
        $this->load->view('index', $page_data); 
	}
	...
	function fee_collect_get_data(){
		...
		$page_data['student_info']=	$this->crud_model->get_student_info($fc_rollid);
	...
	function allfee_collect_get_data(){
		$fc_cid = $this->input->post('get_fcollect_cid');
		...
		$this->load->model('Crud_model'); 
		$page_data['student_info']=	$this->crud_model->get_student_info($fc_rollid);
		//print_r($page_data['student_info']);
		$page_data['fcollectclassdata'] = $this->fee_model->all_getfeeclassdata($fc_cid,$page_data['student_info'][0]['rte'],$page_data['student_info'][0]['student_category']);
		
		
		//$page_data['fcollectalldata'] = $this->fee_model->getfeealldata($fc_cid);
		$page_data['fcollectalldata'] = $this->fee_model->all_getfeealldata($fc_cid,$page_data['student_info'][0]['rte'],$page_data['student_info'][0]['student_category']);
		//echo '<pre>';
		//print_r($page_data);
		...
		//fcollectclassdata
		$this->load->view('allfeecollectdata', $page_data);
	}
	function getString(){
	...
	function fee_collect_process_data_all(){
		...
		if(in_array($pay_mode,array(1,2,3))){
			$data_array['collection_cno']=$fee_collect_data['fee_collection_cno'] = $this->input->post('process_payment_cheque_number');
			$data_array['collection_cdate']= $fee_collect_data['fee_collection_cdate'] = $this->input->post('process_payment_cheque_date');
			$data_array['collection_cbank']= $fee_collect_data['fee_collection_cbank'] = $this->input->post('process_payment_bank_name');
		}
		$data_array['fk_class_id']= $fee_collect_data['fee_collection_class_id'] = $this->input->post('class');
		$data_array['fk_roll_id']=$fee_collect_data['fee_collection_roll_id'] = $this->input->post('roll');
		
		$data_array['payment_mode']=$fee_collect_data['fee_collection_mode'] = $pay_mode;
		$data_array['collection_remarks']=$fee_collect_data['fee_collection_remarks'] = $this->input->post('process_payment_remarks');
  ...
  public function generatereceiptAll($receipt_id) {

	 $this->load->model('fee_model');
	 
	 $receiptdata = $this->fee_model->getReceiptDataAll($receipt_id);
	...
	// $cat_data = $this->fee_model->getfeecategory($cat_id);
	 ...
	 $gwords = explode(".", ($receiptdata->fee_collected + $receiptdata->late_charge));
  ...
  //print_r($receiptdata);
	...
	foreach($data as $key=>$value){
		...
		<td>".($value->fee_collection_amount-$value->discount_amt)."</td>
		        </tr>";
	}
	
	 $pydate = date("d-m-Y",strtotime($receiptdata->fee_callection_date));
	// print_r($receiptdata);
	 $paymode = $receiptdata->payment_mode;
	 $bank_name=$receiptdata->collection_cbank;
	 $sname= $receiptdata->sname;
$cname= $receiptdata->cname;
$fee_collection_remarks= $receiptdata->collection_remarks;
$aname=$receiptdata->aname;
$fee_collection_amount=$receiptdata->fee_collected ;
$fee_collection_late_charge=$receiptdata->late_charge ;
$gtotal=$fee_collection_amount+$fee_collection_late_charge;
...
<table border="0" cellspacing="0" cellpadding="4">
	<tr>
		<th align="left">Receipt No : <span style="font-weight: bold;" >$receiptdata->pk_collections_master</span></th>
...
//$pdf->writeHTML($html, true, false, true, false, '');
 $pdf->writeHTML(str_replace("#COPY","Management Copy",$html), true, false, true, false, '');
 $pdf->AddPage();
 $pdf->writeHTML(str_replace("#COPY","Customer Copy",$html), true, false, true, false, '');
    // ---------------------------------------------------------    
...
$pdf->writeHTML($html, true, false, true, false, '');
  ...
  $pdf->Output($receipt_id.'.pdf', 'I');    
	...
	function feeCollectreceipt($class=0,$stdId=0){
    ...
    $pdf->AddPage(); 
		...
		$this->load->model('Crud_model'); 
		$page_data['student_info']=	$this->crud_model->get_student_info($fc_rollid);
		$stName=$page_data['student_info'][0]['name'];
		$page_data['fcollectclassdata'] = $this->fee_model->getfeeclassdata($fc_cid);
		$page_data['fcollectalldata'] = $this->fee_model->getfeealldata($fc_cid);
		$page_data['fcollectrolldata'] = $this->fee_model->getfeerolldata($fc_cid,$fc_rollid);
		$page_data['class_roll_data'] = array('fc_class_id' => $fc_cid, 'fc_roll_id' => $fc_rollid);
		...
		$array_fee=$array_cat=array();
		
		foreach($page_data['fcollectclassdata'] as $key=>$value){
			 $type=$value->fee_type;
			$array_fee[$type][]=$value->fee_particular_id;
			$array_cat[$value->fee_particular_id]=$value;
		}
		foreach($page_data['fcollectalldata'] as $key=>$value){
						 $type=$value->fee_type;
			$array_fee[$type][]=$value->fee_particular_id;
			$array_cat[$value->fee_particular_id]=$value;
		}
		foreach($page_data['fcollectrolldata'] as $key=>$value){
						 $type=$value->fee_type;
						  $array_fee[$type][]=$value->fee_particular_id;
						  $array_cat[$value->fee_particular_id]=$value;
		}
	$mainTot=0;
	     ...
	     foreach($array_fee as $key=>$value){
			  ...
			  foreach($value as $key1=>$value1){
				...
				$payhisdata = $this->fee_model->getPayData($stdId,$value1);
				  $particulars=$array_cat[$value1];
				 
				  foreach($payhisdata as $key2=>$value2){
				
				$receiptdata = $this->fee_model->getReceiptData($value2->fee_collection_id);
				$recId=$value2->fee_collection_id;
				
				$cat_data = $this->fee_model->getfeecategory($receiptdata['cat_id']);
				
					$mode="";
					$cname=$receiptdata['cname'];
					if($receiptdata['fee_collection_mode']==0){
					$mode="Cash";	
					} else if($receiptdata['fee_collection_mode']==1){
					$mode="Cheque";	
					}
					//pamount
					//echo $receiptdata['fee_collection_amount']-$receiptdata['discount']+$receiptdata['fee_collection_late_charge'];
					//echo '<br />';
					$total=$receiptdata['fee_collection_amount']-$receiptdata['discount']+$receiptdata['fee_collection_late_charge'];
//echo $total; echo '<br />';		
$mainTot+=	$total;	
						...
						$array_data[]=$key;
						if($key==0){
								$htmlConetnt .="<tr><td colspan='8'><b>OTHER</b></td></tr>";
							}
							if($key==1){
							$htmlConetnt .="<tr><td colspan='8'><b>SCHOOL</b></td></tr>";
							}
							if($key==2){
							$htmlConetnt .="<tr><td colspan='8'><b>Traveling</b></td></tr>";
							}
						}
					//}
					if($key==0 && $key2==0){
				// $htmlConetnt .='<tr><td><b>OTHER</b></td></tr>';
				  
			  } elseif($key==1 && $key2==0){
			  //$htmlConetnt .='<tr><td><b>SCHOOL</b></td></tr>';
			  }elseif($key==2 && $key2==0){
			// $htmlConetnt .='<tr><td><b>Traveling</b></td></tr>';
			  }
			  if(!empty($receiptdata['fee_collection_receipt'])){
		$htmlConetnt .="<tr><td align='center'>".$receiptdata['fee_collection_receipt']."</td>
	 <td align='center'>".date("d-m-Y",strtotime($receiptdata['fee_collection_added_date']))."</td>
	 <td align='center'>".$receiptdata['pname']."</td>
	 <td align='center'>".$receiptdata['fee_collection_amount']."</td>
	 <td align='center'>".$receiptdata['discount']."</td>
	 <td align='center'>".$receiptdata['fee_collection_late_charge']."</td>
	 <td align='center'>".number_format($total,2)."</td>
	 <td align='center'>".$mode."</td></tr>";
		  ...
		  $this->load->library("Numbertowords");
		  //$mainTot=number_format($mainTot,2);
		 $gwords = explode(".",$mainTot);
	 ...
	 $numtowrd .= $this->numbertowords->convert_number($c1)." rupees " ;
	 if(!empty($recId)){
	 $receiptdata = $this->fee_model->getReceiptData($recId);
	extract($receiptdata);
	}
	 if($c2 != 0){
		$numtowrd .= " and ".$this->numbertowords->convert_number($c2)." paisa." ; 
...
<table border="0" cellspacing="0" cellpadding="4">
	<tr>
		<th align="left"><span style="font-weight: bold;" >Student Name</span>:$stName</th>
		<th align="right"><span style="font-weight: bold;" > Class</span> : $cname</th>
	</tr>
</table>
</td>
...
<table width="100%">
  <tr>
	 ...
	 <td align='center'>Late charge</td>
...
<table>
   ...
   <td>&nbsp;</td>
    <td>&nbsp;</td>
	 ...
	 <td colspan="6" width="100%" slign="right">$numtowrd</td>
 ...
 $pdf->Output($receipt_id.'.pdf', 'I');   
	...
	function fee_pay_history_all(){
		...
		//$cid=1;
		 ...
		 $payhisdata = $this->fee_model->getPayDataAll($sdnt_id,$cid);
		 
		
		
		$paydata ="";
		
		$paydata .="<table class='table table-bordered'<tr><th>Receipt No </th><th> Pay Amount </th><th> Late Charge </th><th> Mode </th><th> Date </th><th></th><tr>";
		
		foreach($payhisdata as $payhisdata_view){
			$mode = $payhisdata_view->payment_mode;
			...
			$paydata .='<tr><td>'.$payhisdata_view->pk_collections_master.'</td><td>'.$payhisdata_view->fee_collected.'</td><td>'.$payhisdata_view->late_charge.'</td><td>'.$md_val.'</td><td>'.date("d-m-Y",strtotime($payhisdata_view->fee_callection_date)).'</td><td><a target="_new" href="'.base_url().'index.php?admin/generatereceiptAll/'.$payhisdata_view->unique_key.'"><i class="icon-print"></i></a></td></tr>';
	...
	function fee_pay_history(){
		...
		foreach($payhisdata as $payhisdata_view){
			...
			$paydata .='<tr><td>'.$payhisdata_view->fee_collection_receipt.'</td><td>'.$payhisdata_view->fee_collection_amount.'</td><td>'.$payhisdata_view->fee_collection_late_charge.'</td><td>'.$md_val.'</td><td>'.date("d-m-Y",strtotime($payhisdata_view->fee_collection_date)).'</td><td><a target="_new" href="'.base_url().'index.php?admin/generatereceipt/'.$payhisdata_view->fee_collection_id.'"><i class="icon-print"></i></a></td></tr>';
   ...
   /* ////////////////////// Fee Reporting //////////////////////// */
	...
	function feereportingall(){
		...
		@extract($pdata);
		  ...
		  $ptype = $fee_particular_type;
		 // echo $fee_applicble; exit;
		  if($ptype != 'fp_roll'){
			  
			  $page_data['student_data'] = $this->fee_model->getStudentReportAll($cid,$fee_applicble);
			  
		  } else if($ptype == 'fp_roll'){
			   $page_data['student_data'] = $this->fee_model->getStudentRollReportAll($fee_particular_type_id,$fee_applicble);
		  }
		}
        ...
        $page_data['page_name']  = 'feereportingall';
        $page_data['page_title'] = get_phrase('fee_reporting_all');
        $this->load->view('index', $page_data);
	}
	function income_expenses($id){
		$this->load->model('fee_model');
	        if($id>0){
				
				$page_data['data']=$this->fee_model->getIncomeExpence($id); 
				// 
			}
			//print_r($this->fee_model->getIncomeExpence());
			$page_data['data_all']=$this->fee_model->getIncomeExpence(); 
			$page_data['page_name']  = 'income_expenses';
			$page_data['page_title'] = get_phrase('income_expenses');
			$this->load->view('index', $page_data);
	}
	function del_income_expenses($id){
		$this->load->model('fee_model');
		$this->fee_model->deleteIncomeExpence($id);
		redirect(base_url().'index.php?admin/income_expenses','refresh');
...
';
   ...
   function caldays(){
    ...
    function sms_student_data_all(){ 
	 ...
	 // $this->db->select('name,father_name,parent_phone1');
	  //$this->db->where('class_id',$cid);
	  //$query = $this->db->get_where('student');
	  //$data_student= $query->row_array();
		$this->db->select('name,father_name,parent_phone1');
	  	$this->db->where('class_id',$cid);
		$query =  $this->db->get_where('student');
		$dataPhone=$query->result();
		$str="";
		foreach($dataPhone as $key=>$value){
			$phoneNumber=$value->parent_phone1; 
			if(preg_match('/^\d{10}$/',$phoneNumber)) // phone number is valid
			{
			$phoneNumber = '90' . $phoneNumber;
                 $str .='<input type="hidden" name="phone_No[]" value="'.$phoneNumber.'"/>';
	 ...
	 echo '<div class="modal-header">
			...
			<h3 id="custom-tam-mode">Send SMS to '.$name.' (Class Name)</h3>
			</div>
			...
			<div id="sms-feedback"></div>
				...
				<button type="button" class="btn btn-success" id="sndsmsbtnbulk">Send SMS</button>
				</div>
				</div>
				</form>
   ...
   function sms_student_snd_all(){
	   $smsg = $this->input->post('smessage');
	   
	  if(count($_POST['phone_No'])){
		  
		 //$res =  
		 $vals=array();
		 foreach($_POST['phone_No'] as $key=>$value){
			
			 $vals[]=send_tam_sms_all($value,$smsg);
		 }
		 if(in_array(1,$vals)){
			 echo "true";
		 }else{
