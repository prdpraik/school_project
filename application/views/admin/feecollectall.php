...
$(document).on('keyup','.late_charge',function(e){
	$tot=0;
	$('.late_charge').each(function($i){ 
	   $tot +=Number($(this).val());
	});
	$('.late_charge_span').html($tot);
});
...
$(document).on('change','#process_payment_mode',function(){
	...
	if(mode_val == 1 || mode_val == 2 || mode_val == 3){
		//alert(mode_val);
		if(mode_val==1){
			$('.lbl_nbr').html('<?php echo get_phrase('cheque_number');?>');
		} else if(mode_val==3){
			$('.lbl_nbr').html('<?php echo get_phrase('ref_number');?>');
		} else{
			$('.lbl_nbr').html('Card Number');
		}
		if(mode_val==1){
			$('.lbl_date').html('<?php echo get_phrase('cheque_date');?>');
		} else if(mode_val==3){
			$('.lbl_date').html('<?php echo get_phrase('Date');?>');
		} else{
			$('.lbl_date').html('Card Exp Date');
		}
		if(mode_val==1 || mode_val==3){
			$('.lbl_name').html('<?php echo get_phrase('bank_name');?>');
		}else{
			$('.lbl_name').html('Name in card');
		}
		$('#custom-tam-cheque-info').show();
...
function call_data_load(){
	...
	$.ajax({
							type: 'POST',
							url: '<?php echo base_url(); ?>index.php?admin/allfee_collect_get_data/',
...
$(document).on('click','.custom-fee-pay-his-btn-all',function(e){
	...
	$('#custom-tam-model-body-pay-his').text('');
		...
		$.ajax({
							type: 'POST',
							url: '<?php echo base_url(); ?>index.php?admin/fee_pay_history_all/',
							data: { sid:sid , cid:class_id },
							success: function(data) {
								//alert(data);
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$(document).on("keydown",".only_number1",function (event) {
