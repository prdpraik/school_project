            ...
            <?php if(!empty($sel_data)) @extract($sel_data); ?>
			   ...
			   <div class="span3">
			       <div class="control-group">
                        ...
                        <div class="controls">
                        	<select name="fee_collect_roll" id="fee_collect_roll" class="span8" >
               ...
               <div class="span3">
                        ...
                        <div class="controls">
                        	<select name="fee_report_particular" id="fee_report_particular" class="span8" >
                            	<option value="">--- select class first---</option>
                            </select>
...
function object_to_array($object) {
 if (is_object($object)) {
  ...
  return $object;
					 ...
					 //@extract($ptr_data);
					$ptr_data_new=object_to_array($ptr_data);print_r($ptr_data_new); 
					 $ci =& get_instance();  
					 
					 $ci->load->model('fee_model');
					foreach($ptr_data_new as $key=>$value){
						//echo $value->fcategory;
						print_r($value['fee_particular_id']);
					}
					 $fperiod_data = $ci->fee_model->getfeepperiodAll(22);
					 
					//echo '<pre>'; print_r($ptr_data);echo '</pre>';
                    ...
                    <table cellpadding="0" cellspacing="0" border="0" class="responsive" id="feereport_dtable">
                                        ...
                                        <thead>
                                            ...
                                            <tr>
                                                ...
                                                <?Php if(empty($_POST['fee_collect_roll'])){ ?>
                                                <th><div><?php echo get_phrase('student_name'); ?></div></th>
												<?php } ?>
												 <?Php if(empty($_POST['fee_report_particular'])){ ?>
												  <th><div><?php echo get_phrase('Particular'); ?></div></th>
												 <?php } ?>
...
function loadStudent(fcollect_cid,selected){
	...
	$.ajax({
							...
							data: { get_fcollect_cid:fcollect_cid,selected:selected },
...
$('#fee_report_class_id').on('change',function(){
	...
	call_pter(report_cid,0);
	loadStudent(report_cid,0)
	
});
//call_pter(<?php echo sprintf("%d",$_POST['fee_report_class_id']);?>,<?php echo sprintf("%d",$_POST['fee_report_particular']);?>); 
function call_pter(report_cid,sel_val){
	//alert(report_cid+'-'+sel_val);
	$.ajax({
							type: 'POST',
							url: '<?php echo base_url(); ?>index.php?admin/fee_report_get_particular/',
							data: { get_report_cid:report_cid,selected:sel_val},
							success: function(data) {
								//alert(data);
...
$(window).load(function(){
	...
	call_pter(<?php echo sprintf("%d",$_POST['fee_report_class_id']);?>,<?php echo sprintf("%d",$_POST['fee_report_particular']);?>); 
	loadStudent(<?php echo sprintf("%d",$_POST['fee_report_class_id']);?>,<?php echo sprintf("%d",$_POST['fee_collect_roll']);?>); 
